...
import android.content.LilyEventLog;
...
public class MobileSignalController extends SignalController<
    ...
    private int mLastConnectLevel = -1;
    private int mPreviousSignalLevel = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
    ...
    public MobileSignalController(Context context, Config config, boolean hasMobileData,
        ...
        //         Settings.Global.getUriFor("incalluiMode"), false, mIncallUIShowObserver, ActivityManager.getCurrentUser());
    }

     //update by wentao 20190827 for LILYDEF-961 begin
    //xushaohua add by 20190723
    // protected final ContentObserver mIncallUIShowObserver = new ContentObserver(new Handler(mContext.getMainLooper())) {
    //     @Override
    //     public void onChange(boolean selfChange) {
    //         incalluiMode = Settings.Global.getInt(mContext.getContentResolver(),"incalluiMode", 0)==1;
    //         Intent newIntent = new Intent();
    //         newIntent.setAction(TelephonyIntents.SPN_STRINGS_UPDATED_ACTION);
    //         handleBroadcast(newIntent);
    //         Log.d("wentao","mobile signalcontrol call change begin");
    //         updateTelephony();
    //         Log.d("wentao","mobile signalcontrol call change begin");
    //     }
    // };
    public void updateInCallUi(boolean incall){
            incalluiMode = incall;
    ...
    //end
     //update by wentao 20190827 for LILYDEF-961 end
    ...
    public void unregisterListener() {
        ...
        //mContext.getContentResolver().unregisterContentObserver(mIncallUIShowObserver);
    }
    ...
    public void notifyListeners(SignalCallback callback) {
        ...
        callback.updateStateIcon(statusIcon,mSubscriptionInfo.getSubscriptionId(),icons.mSingleActivtyIcon);
    ...
    private void generateIconGroup() {
        ...
        singleSignalIcon = TelephonyIcons.getSignalStrengthIcon(slotId, inet, level, roaming, incalluiMode);

        singleActivtyIcon = TelephonyIcons.getActivitySignalStrengthIcon(level, incalluiMode);
        ...
        int sbDiscState = TelephonyIcons.getSignalNullIcon(slotId,incalluiMode);
        if (DEBUG) {
            Log.d(mTag, "singleSignalIcon=" + getResourceName(singleSignalIcon)
        ...
        mCurrentState.iconGroup = new MobileIconGroup(
                ...
                singleSignalIcon, stackedDataIcon, stackedVoiceIcon, dataActivityId,singleActivtyIcon);
    }
    ...
    private void insertAttennaEventLog(){
        int currentSignalLevel = mSignalStrength.getLevel();
        Log.d(mTag, "mCurrentState.level =" + mCurrentState.level + " mLastConnectLevel =" + mLastConnectLevel
                + " mPreviousSignalLevel=" + mPreviousSignalLevel + " currentSignalLevel=" + currentSignalLevel);
        if(mCurrentState.level != mLastConnectLevel){              
            switch(mCurrentState.level){
                case 4:
                    mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_ANTENNA_LEVEL4);
                    break;
                case 3:
                    mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_ANTENNA_LEVEL3);
                    break;
                case 2:
                    mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_ANTENNA_LEVEL2);
                    break;
                case 1:
                    mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_ANTENNA_LEVEL1);
                    break;
                case 0:
                    mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_ANTENNA_LEVEL0);
                    break;
                default:
                    
            }
            mLastConnectLevel = mCurrentState.level; 
        }
            
        if(mSignalStrength != null && mSignalStrength.getLevel() != mPreviousSignalLevel){
            if(mSignalStrength.getLevel() == SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN){
                    // out of coverage
                mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_OUT_OF_COVERAGE);
            } else if (mPreviousSignalLevel == SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN){
                    // in coverage
                mContext.getContentResolver().insertEventLog(LilyEventLog.EVENT_LOG_ID_IN_COVERAGE);
            }
        }
        mPreviousSignalLevel = currentSignalLevel;
    }
    ...
    static class MobileIconGroup extends SignalController.IconGroup {
        ...
        public MobileIconGroup(String name, int[][] sbIcons, int[][] qsIcons, int[] contentDesc,
                ...
                int stackedVoicelIcon, int activityId ,int singleActivtyIcon) {
            ...
            mSingleActivtyIcon = singleActivtyIcon;
        }
    }
