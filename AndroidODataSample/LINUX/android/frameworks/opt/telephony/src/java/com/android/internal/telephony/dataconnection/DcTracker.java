...
public class DcTracker extends Handler {
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver () {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } //update imsapn add by cqq
		 else if (action.equals("com.android.set.data.profile")) {
                android.util.Log.d("IMSAPN","action.equals(com.android.set.data.profile)");
                ims = intent.getStringExtra("ims");
                onActionIntentSetDataProfile(intent);
            }else {
	...
	/*update imsapn add by cqq*/
	protected void onActionIntentSetDataProfile(Intent intent) {
		android.util.Log.i("IMSAPN","onActionIntentSetDataProfile");
		Message msg = obtainMessage(DctConstants.EVENT_SET_DATA_PROFILE,intent.getAction());
		sendMessage(msg);
	}
	...
	/*update imsapn add by cqq*/
        filter.addAction("com.android.set.data.profile");
        android.util.Log.d("IMSAPN","filter.addAction(com.android.set.data.profile);");
	...
	/*update imsapn add by cqq*/
		static String ims = null;
		protected void setDataProfiles() {
			android.util.Log.d("IMSAPN","into setDataProfiles");
			if (mAllApnSettings != null && !mAllApnSettings.isEmpty()) {
				ArrayList<DataProfile> dps = new ArrayList<DataProfile>();
				for (ApnSetting apn : mAllApnSettings) {
                if (apn.modemCognitive) {
				...
				android.util.Log.d("IMSAPN","setDataProfiles");
				android.util.Log.d("IMSAPN","ims:"+ims);
				/*DataProfile mDataProfile = (DataProfile)dps.get(2);*///value = ""
				DataProfile dd = new DataProfile(
				2,
				ims,
				"IPV6",
				0,
				...
				android.util.Log.d("IMSAPN","ims update ...");
	
				DataProfile[] mDataProfile1 = new DataProfile[1];
				mDataProfile1[0]=dd;
				android.util.Log.d("IMSAPN","mDataProfile1:"+mDataProfile1[0]);
				mPhone.mCi.setDataProfile(mDataProfile1,mPhone.getServiceState().getDataRoamingFromRegistration(),null);
				android.util.Log.d("IMSAPN","ims update ...end");
			}
		}
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
			...
			/*update imsapn add by cqq*/
	    	case DctConstants.EVENT_SET_DATA_PROFILE:
		   		android.util.Log.d("IMSAPN","case DctConstants.EVENT_SET_DATA_PROFILE");
		   		setDataProfiles();
		   		break;
